...
export function LoadingState({ message = '加载中...' }: { message?: string }) {
  return <div className="glass-surface px-4 py-3 text-sm text-[var(--glass-text-secondary)]">{message}</div>
}

export function ErrorState({ message = '加载失败。' }: { message?: string }) {
  return <div className="glass-surface glass-danger rounded-xl px-4 py-3 text-sm">{message}</div>
}
