...
ASSET_GENERATE_STEP_KEY = "asset_generate"
ASSET_MODIFY_STEP_KEY = "asset_modify"


def run_asset_generate_task(task_id: str) -> None:
    _run_asset_task(task_id, workflow_kind="generate")


def run_asset_modify_task(task_id: str) -> None:
    _run_asset_task(task_id, workflow_kind="modify")


def _run_asset_task(task_id: str, *, workflow_kind: str) -> None:
    db = SessionLocal()
    try:
        ...
        step_key = ASSET_GENERATE_STEP_KEY if workflow_kind == "generate" else ASSET_MODIFY_STEP_KEY
        step_title = "Asset Generate" if workflow_kind == "generate" else "Asset Modify"
        ...
        output = _build_asset_output(db, refreshed_task, workflow_kind=workflow_kind)
...
def _build_asset_output(db, task: Task, *, workflow_kind: str) -> dict:
    service = AssetService(db)
    asset = service.get_task_asset(task.user_id, task.id)
    ...
    if workflow_kind == "modify":
        ...
        asset = service.update_project_asset(
            task.user_id,
            asset.id,
            description=prompt or asset.description,
            preview_media_id=str(preview_media_id) if isinstance(preview_media_id, str) else None,
            set_description=bool(prompt),
            set_preview_media="preview_media_id" in payload,
        )
        mode = "modified"
    else:
        mode = "generated"
